...
impl ProxyService {
    ...
    /// Clone the shared Claude uploaded-auth manager from the running server.
    /// Returns `None` if the server is not currently running.
    pub async fn clone_claude_uploaded_auth_manager(
        &self,
    ) -> Option<crate::services::oauth_refresh::ClaudeUploadedAuthManager> {
    ...
    ///
    /// Why: `/api/status` renders provider quota from `rate_limits`; a snapshot
    /// for a deleted provider would surface as a ghost entry. Calling this on
    /// provider deletion keeps both the in-memory store and the DB persistence
    /// consistent with the provider list.
